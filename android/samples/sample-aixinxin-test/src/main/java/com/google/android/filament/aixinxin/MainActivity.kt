...
class MainActivity : Activity() {
    ...
    private fun createDefaultRenderables() {
        val buffer = assets.open("models/bodyLow/AiXinXin_Rig_bodyCloth_low_v01.gltf").use { input ->
//        val buffer = assets.open("models/headLow/AiXinXin_Rig_head_low_v01.gltf").use { input ->
            val bytes = ByteArray(input.available())
            input.read(bytes)
            ByteBuffer.wrap(bytes)
        }

        modelViewer.loadModelGltfAsync(buffer) { uri -> readCompressedAsset("models/bodyLow/$uri") }
//        modelViewer.loadModelGltfAsync(buffer) { uri -> readCompressedAsset("models/headLow/$uri") }
        updateRootTransform()
    }
